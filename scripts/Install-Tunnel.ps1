[CmdletBinding()]
param(
    [string] $InstallRoot = (Join-Path $env:LOCALAPPDATA 'RemoteImageBridge')
)
...
try {
    $release = Invoke-RestMethod -Uri ('https://api.github.com/repos/cloudflare/cloudflared/releases/tags/' + $version) -Headers @{ 'User-Agent' = 'RemoteImageBridge-installer' } -TimeoutSec 30
    ...
    Invoke-WebRequest -UseBasicParsing -Uri $downloadUrl -OutFile $temporaryPath -TimeoutSec 120
